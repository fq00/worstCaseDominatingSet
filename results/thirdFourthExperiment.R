...
data1 = read.csv2("thirdExperiment.txt", head = TRUE, sep = " ")
data2 = read.csv2("fourthExperiment.txt", head = TRUE, sep = " ")

# make data

power_exponent = as.numeric(as.character(data2$power_exponent))
Greedy_result = as.numeric(as.character(data2$covering_size))
EA_result_mean = as.numeric(lapply(split(data1$covering_size, data1$power_exponent), mean))
EA_result_sd = as.numeric(lapply(split(data1$covering_size, data1$power_exponent), sd))

dataPlot = data.frame(x = power_exponent, y = Greedy_result, z = EA_result_mean, w = EA_result_sd)
rm(power_exponent, Greedy_result, EA_result_mean, EA_result_sd)

# perform correlation

mod1 = lm(dataPlot$y ~ dataPlot$x)
mod2 = lm(dataPlot$z ~ I(dataPlot$x^(1/2)) + I(log(dataPlot$x)))
